...
// --- Selection Component ---
// This handles the local selection for each product in the grid
const ProductCard = ({ product, onAddToCart }: { product: Product, onAddToCart: (p: Product, s: string, c: string) => void }) => {
  ...
  return (
    <div className="group">
      <div className="relative aspect-[4/5] overflow-hidden border border-zinc-100 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-900/40">
        <img src={product.img} alt={product.name} className="h-full w-full object-contain p-8 transition-transform duration-700 group-hover:scale-110" />
      </div>
      <div className="mt-6 space-y-3">
        <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-400">{product.category}</p>
        <div className="flex items-center justify-between">
          <h4 className="text-[11px] font-black uppercase tracking-widest">{product.name}</h4>
          <p className="text-[11px] font-bold italic">₱{product.price}</p>
        </div>
        ...
        <div className="flex gap-2 py-1">
          {product.colors.map((color) => (
            <button
              ...
              className={`h-4 w-4 rounded-full border transition-all ${selectedColor.name === color.name ? 'ring-1 ring-offset-2 ring-red-600' : 'border-transparent'}`}
        ...
        <div className="flex flex-wrap gap-1">
          {product.sizes.map((size) => (
            <button
              ...
              className={`border px-2 py-1 text-[8px] font-black transition-all ${selectedSize === size ? 'bg-foreground text-background border-foreground' : 'border-zinc-200 dark:border-zinc-800 hover:border-red-600'}`}
            >
              {size}
            </button>
          ))}
        ...
        <button 
          ...
          className="mt-2 w-full border border-zinc-950/10 py-3 text-[9px] font-black uppercase tracking-[0.2em] transition-all hover:bg-foreground hover:text-background dark:border-white/10"
        >
          Add to Bag +
        </button>
      </div>
...
export default function Home() {
  ...
  const [formData, setFormData] = useState({ email: "", fullName: "", address: "", phone: "" });
  const [cart, setCart] = useState<CartItem[]>([]);

  useEffect(() => setMounted(true), []);

  // --- Updated Product Data with Sizes/Colors ---
  const products: Product[] = [
    ...
    { 
      ...
      sizes: ["S", "M", "L", "XL"],
      colors: [{ name: "Cream", hex: "#F5F5DC" }, { name: "Black", hex: "#000000" }]
    },
    { 
    ...
    { 
      ...
      sizes: ["S", "M", "L"],
      colors: [{ name: "Charcoal", hex: "#333333" }, { name: "Red", hex: "#DC2626" }]
    },
  ];

  const toggleTheme = () => setTheme(theme === "dark" ? "light" : "dark");

  // --- Updated Cart Functions to handle variants ---
  ...
  return (
    <div className={`min-h-screen transition-colors duration-700 ease-in-out bg-background text-foreground ${isCartOpen || isCheckoutOpen ? 'overflow-hidden' : ''}`}>
      ...
      <nav className="sticky top-0 z-50 w-full border-b border-zinc-950/5 bg-background/80 backdrop-blur-xl dark:border-white/5">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-6">
            <button onClick={toggleTheme} className="p-2 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all duration-300">
              {theme === "dark" ? <Sun size={18} className="text-yellow-400" /> : <Moon size={18} className="text-zinc-900" />}
            </button>
                      ...
                      <div className="flex flex-1 flex-col justify-between">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center border border-zinc-200 dark:border-zinc-800">
                            <button onClick={() => updateQuantity(item.id, item.selectedSize, item.selectedColor, -1)} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-900"><Minus size={10} /></button>
                            <span className="px-3 text-[10px] font-bold">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.id, item.selectedSize, item.selectedColor, 1)} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-900"><Plus size={10} /></button>
                          </div>
      ...
      {/* --- Checkout Modal & Footer (Kept Exactly Same) --- */}
      {isCheckoutOpen && (
            ...
            {!orderComplete ? (
              <form onSubmit={handleCheckoutSubmit} className="grid md:grid-cols-2 gap-12">
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-2xl font-black italic uppercase tracking-tighter text-foreground">Shipping details</h3>
                    <button type="button" onClick={() => handleCheckoutSubmit()} className="flex items-center gap-2 text-[8px] font-black uppercase bg-zinc-200 dark:bg-zinc-800 px-3 py-1.5 rounded hover:bg-red-600 hover:text-white transition-colors text-zinc-900 dark:text-zinc-100">
                      <Beaker size={12} /> Test Email
                    </button>
                  </div>
                  <div className="space-y-4">
                    <input type="text" placeholder="Full Name" required className="w-full bg-zinc-100 dark:bg-zinc-900 p-4 rounded text-[10px] font-bold uppercase tracking-widest border-none outline-none focus:ring-1 focus:ring-red-600 text-black dark:text-white placeholder:text-zinc-500" onChange={e => setFormData({...formData, fullName: e.target.value})} />
                    <input type="email" placeholder="Email" required className="w-full bg-zinc-100 dark:bg-zinc-900 p-4 rounded text-[10px] font-bold uppercase tracking-widest border-none outline-none focus:ring-1 focus:ring-red-600 text-black dark:text-white placeholder:text-zinc-500" onChange={e => setFormData({...formData, email: e.target.value})} />
                    <input type="text" placeholder="Phone Number" required className="w-full bg-zinc-100 dark:bg-zinc-900 p-4 rounded text-[10px] font-bold uppercase tracking-widest border-none outline-none focus:ring-1 focus:ring-red-600 text-black dark:text-white placeholder:text-zinc-500" onChange={e => setFormData({...formData, phone: e.target.value})} />
                    <textarea placeholder="Full Shipping Address" required className="w-full bg-zinc-100 dark:bg-zinc-900 p-4 rounded text-[10px] font-bold uppercase tracking-widest h-32 border-none outline-none focus:ring-1 focus:ring-red-600 text-black dark:text-white placeholder:text-zinc-500" onChange={e => setFormData({...formData, address: e.target.value})} />
                  </div>
                </div>
                <div className="flex flex-col items-center justify-center text-center border-l border-zinc-500/10 pl-8">
                  <div className="flex items-center gap-2 text-blue-500 mb-6 uppercase font-black text-[10px] tracking-widest"><Smartphone size={16} /> GCash QR Payment</div>
                  ...
                  <button disabled={isProcessing} type="submit" className="w-full bg-blue-600 text-white py-5 rounded font-black uppercase text-[10px] tracking-[0.3em] hover:bg-blue-700 transition-all disabled:opacity-50">
              ...
              <div className="py-20 text-center">
                ...
                <h3 className="text-3xl font-black italic uppercase tracking-tighter mb-4 text-foreground">Payment Received</h3>
                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.2em]">A receipt has been sent to {formData.email}</p>
                <p className="mt-12 text-[8px] font-black uppercase tracking-widest animate-pulse text-foreground">Returning to store...</p>
              </div>
            )}
          </div>
        </div>
      ...
      <footer className="border-t border-zinc-100 bg-background px-8 py-24 transition-colors duration-700 dark:border-white/5">
        <div className="mx-auto max-w-7xl text-center md:text-left flex flex-col md:flex-row justify-between gap-12">
          <div>
            <h1 className="mb-2 text-4xl font-black italic tracking-tighter uppercase">ARC APPAREL</h1>
            <p className="max-w-[200px] text-[9px] font-bold leading-relaxed tracking-widest text-zinc-400 uppercase">Redefining street aesthetics through minimal design.</p>
          </div>
          <div className="flex flex-col items-center gap-6 md:items-end">
             <div className="flex h-12 w-12 items-center justify-center rounded-full border border-zinc-100 dark:border-white/10"><Globe size={18} /></div>
             <span className="text-[9px] font-black uppercase tracking-[0.4em] text-zinc-500">© 2026 ARC APPAREL GROUP</span>
          </div>
        </div>
      </footer>
    </div>
